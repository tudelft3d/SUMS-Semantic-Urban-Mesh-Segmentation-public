...
//
// $URL: https://github.com/CGAL/cgal/blob/releases/CGAL-4.14/Classification/include/CGAL/Classification/classify.h $
// $Id: classify.h f5836b2 %aI Laurent Rineau
...
namespace internal {
  ...
  class Classify_functor
  {
    ...
    std::vector<float>* m_outvotes;//***************************Weixiao Update***********************************
    std::vector<float>* m_outprob;//***************************Weixiao Update***
  ...
  public:
    ...
    Classify_functor (const Label_set& labels,
	  ...
	  m_out (out),
      m_outvotes(nullptr),//***************************Weixiao Update***********************************
      m_outprob(nullptr)//***************************Weixiao Update*********************
    { }
    ...
    {
      //*******Weixiao Update**************
      std::vector<float>& tmp = *m_outvotes;
      std::vector<float>& tmp_prob = *m_outprob;
      //***********************************
      ...
      {
		//*******Weixiao Update*********
        tmp_prob[m_labels.size() * s + k] = values[k];
        //******************************
...
/// \endcond
  ...
  */
  ...
  //*********************Weixiao Update With Value Return********************************** 
  template <typename ConcurrencyTag,
      typename ItemRange,
      typename Classifier,
      typename LabelIndexRange>
      void classify(const ItemRange& input,
          ...
          std::vector<float>& outprob)
  {
	  //Given predefined size for Intel-TBB parallel processing
      //Warning: no size pre-define or using push_back will cause unexpected errors!!!
      output.resize(input.size());
      outvotes.resize(input.size());
	  outprob.resize(input.size() * labels.size());
	  
      internal::Classify_functor<Classifier, LabelIndexRange>
          f(labels, classifier, output, outvotes, outprob);
