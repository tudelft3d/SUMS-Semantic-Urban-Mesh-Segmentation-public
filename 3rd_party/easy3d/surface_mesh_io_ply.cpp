	...
	namespace io {
		...
		namespace details {
			...
			inline void add_vertex_properties(SurfaceMesh* mesh, const std::vector<PropertyT>& properties)
			{
				for (const auto& p : properties) {
					std::string name = p.property_name;
		...
		bool load_ply(const std::string& file_name, SurfaceMesh* mesh)
		{
			...
			for (std::size_t i = 0; i < elements.size(); ++i) {
				...
				if (e.element_name == "vertex") {
					FloatProperty x_coords, y_coords, z_coords;
			...
			for (std::size_t i = 0; i < elements.size(); ++i) {
				...
				if (e.element_name == "vertex") {
					...
					std::cerr << "element \'" << e.element_name << "\' ignored" << std::endl;
			}
		...
		bool save_ply(const std::string& file_name, const SurfaceMesh* mesh, const std::vector<std::string> &comment, bool binary)
		{
			...
			face_vertex_indices.property_name = "vertex_indices";
				...
				element_edge.float_list_properties.size() > 0)
			{
				// vertex_indices
				IntListProperty edge_vertex_indices;
				edge_vertex_indices.property_name = "vertex_indices";
			...
			PlyWriter writer;
			return writer.write(file_name, elements, comment, binary);
		}
		//-------------------------------------------------------------------------------------------------------//
