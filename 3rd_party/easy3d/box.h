...
namespace easy3d {
    ...
    public:
		...
		//Weixiao Update//
		bool IsNull() const { return x_min() > x_max() || y_min() > y_max(); }
		//*************//
    ...
    public:
        ...
        void clear() { initialized_ = false; }
		//**************************Weixiao Update**************************//
		bool IsNull() const { return x_min() > x_max() || y_min() > y_max() || z_min() > z_max(); }
		void Set(const Vec<3, float> &p)
		{
			min() = max() = p;
		}

		void Offset(const float &delta)
		{
			x_min_ -= delta;
			y_min_ -= delta;
			z_min_ -= delta;
		...
		bool Collide(GenericBox3<FT> const &b) const
		{
			return b.x_min() < x_max_ && b.x_max() > x_min_ &&
				b.y_min() < y_max_ && b.y_max() > y_min_ &&
				b.z_min() < z_max_ && b.z_max() > z_min_;
		}
		...
		void Intersect(const GenericBox3<FT> & b)
		{
			if (x_min_ < b.x_min()) x_min_ = b.x_min();
			if (y_min_ < b.y_min()) y_min_ = b.y_min();
			if (z_min_ < b.z_min()) z_min_ = b.z_min();

			if (x_max_ > b.x_max()) x_max_ = b.x_max();
			if (y_max_ > b.y_max()) y_max_ = b.y_max();
			if (z_max_ > b.z_max()) z_max_ = b.z_max();

			if (x_min_ > x_max_ || y_min_ > y_max_ || z_min_ > z_max_) SetNull();
		}
		//*****************************************************************//
