...
namespace easy3d {
    ...
    float PointCloudSimplification::average_space(PointCloud *cloud, KdTree *tree, int k, bool accurate,
                                                    int samples) {
        KdTree *kdtree = tree;
    ...
    PointCloudSimplification::uniform_simplification(PointCloud *cloud, float epsilon, KdTree *tree) {
        KdTree *kdtree = tree;
        bool need_delete(false);
        if (!kdtree) {
            kdtree = new KdTree;
        ...
        for (std::size_t i = 0; i < points.size(); ++i) {
            if (keep[i]) {
                const vec3 &p = points[i];
                std::vector<int> neighbors;
                kdtree->find_points_in_radius(p, sqr_dist, neighbors);
                if (neighbors.size() > 1) {
