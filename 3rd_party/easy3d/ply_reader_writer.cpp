...
#define EDGE	"edge"
			...
			bool binary /* = false */) const
		{
			...
			// But I want to store segmentation information, which requires a large range (eq. the 
			// vertex indices), so I choose unsigned int.
			e_ply_type length_type = PLY_UINT32;

			for (std::size_t i = 0; i < elements.size(); ++i) {
				const std::string& element_name = elements[i].element_name;
				std::size_t num = elements[i].num_instances;
				...
				for (std::size_t j = 0; j < int_list_properties.size(); ++j) {
					const std::string& name = int_list_properties[j].property_name;
				...
				for (std::size_t j = 0; j < float_list_properties.size(); ++j) {
					const std::string& name = float_list_properties[j].property_name;
				...
				for (std::size_t j = 0; j < vector_properties.size(); ++j) {
					const std::string& name = vector_properties[j].property_name;
				...
				for (std::size_t j = 0; j < float_properties.size(); ++j) {
					const std::string& name = float_properties[j].property_name;
				...
				for (std::size_t j = 0; j < int_properties.size(); ++j) {
					const std::string& name = int_properties[j].property_name;
			...
			bool binary /* = false */) const
		{
			...
			if (!ply_add_comment(ply, "saved by gaoweixiaocuhk@gmail.com")) {
				std::cerr << "failed to add comment" << std::endl;
				ply_close(ply);
				return false;
			}
		...
		namespace details {
			...
			inline void convert(const GenericProperty<VT_Input>& input, GenericProperty<VT_Output>& output) {
				...
				output.property_name = input.property_name;
			...
			template <typename PropertyT>
			bool extract_vector_property(std::vector< PropertyT >& properties,
				...
				Vec3Property& prop)
			{
					...
					details::extract_named_property(properties, z_coords, z_name))
				{
					...
					prop.resize(num);
					for (std::size_t j = 0; j < num; ++j)
						prop[j] = vec3(x_coords[j], y_coords[j], z_coords[j]);
					return true;
				}
				else
					return false;
		...
		void PlyReader::collect_elements(std::vector<Element>& elements) const {
			...
			for (auto prop : list_properties_) {
				...
				for (auto& e : elements) {
					if (e.element_name == prop->element_name) {
						element = &e;
						break;
					}
				}
			...
			for (auto& element : elements) {
				...
				Vec3Property prop_color(element.element_name, "color");
				if (details::extract_vector_property(element.float_properties, "r", "g", "b", prop_color))
					...
					// if , we ignore the "alpha" values
					IntProperty alpha;
					details::extract_named_property(element.int_properties, alpha, "alpha");
				}
			}
		}
