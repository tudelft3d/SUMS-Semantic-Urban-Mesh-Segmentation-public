...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Hessigsim3D dataset.
...
COLOR_MAP = np.asarray(
    [
        [178, 203, 47],  #C00  Low Vegetation
        [183, 178, 170],  #C01  Impervious Surface
        [32, 151, 163],  #C02  Vehicle
        [168, 33, 107],  #C03  Urban Furniture
        [255, 122, 89],  #C04  Roof
        [255, 215, 136],  #C05  Facade
        [89, 125, 53],  #C06  Shrub
        [0, 128, 65],  #C07  Tree
        [170, 85, 0],  #C08  Soil/Gravel
        [252, 225, 5],  #C09  Vertical Surface
        [128, 0, 0],  #C10  Chimney
    ]
)

def transfer_16bit_to_8bit(array_in):
    min_16bit = np.min(array_in)
    max_16bit = np.max(array_in)
    array_8bit = np.array(np.rint(255 * ((array_in - min_16bit) / (max_16bit - min_16bit))), dtype=np.uint8)
    return array_8bit

def read_las(filename, has_label = True, add_las_feas = True):
    ...
    # apply global shift (-513852.00, -5426490.00, -224.51)
    x += -513852.00
    y += -5426490.00
    z += -224.51

    xyz = np.hstack((x, y, z)).astype('f8')

    r = transfer_16bit_to_8bit(np.reshape(inFile.get_red(), (N_points, 1)))
    g = transfer_16bit_to_8bit(np.reshape(inFile.get_green(), (N_points, 1)))
    b = transfer_16bit_to_8bit(np.reshape(inFile.get_blue(), (N_points, 1)))
    rgb = np.hstack((r, g, b))

    if add_las_feas:
        return_num = np.reshape(inFile.num_returns, (N_points, 1))
        num_of_return = np.reshape(inFile.return_num, (N_points, 1))
        reflec = np.reshape(inFile.Reflectance, (N_points, 1))
        feas = np.stack((return_num, num_of_return, reflec), axis=1).astype('f8')
        feas = np.transpose(np.concatenate(feas, axis=-1))
    if has_label:
        labels = np.reshape(inFile.Classification, (N_points, 1))
        labels = np.concatenate(labels, axis=-1)
        if add_las_feas:
            return xyz, rgb, labels, feas
        else:
...
class Hessigsim3DDataset(PointCloudDataset):
    """Class to handle Hessigsim3D dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'Hessigsim3D')
        ...
        self.label_to_names = {0: "Low Vegetation",
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for Hessigsim3D data: ', self.set)
        ...
        for folder in folders:
            data_folder = self.path + "/raw/" + folder
            ...
            if self.set == 'training' and folder == "train/":
                ...
                break
            elif self.set == 'validation' and folder == "validate/":
                self.files += files
                for file in files:
                    file_name = os.path.splitext(os.path.basename(file))[0]
                    self.cloud_names.append(file_name)
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        elif self.config.in_features_dim == 5 or self.config.in_features_dim == 8:
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_feas = np.vstack((data['red'], data['green'], data['blue'],
                                        data['ReturnNumber'], data['NumberOfReturns'], data['Reflectance'])).T
                ...
                # Read ply file
                if self.set != 'test':
                    xyz, rgb, labels, las_feas = read_las(file_path)
                    labels = np.array(labels)
                else:
                    xyz, rgb, las_feas = read_las(file_path, False, True)
                    labels = np.zeros(len(rgb))

                sub_feas = np.hstack((rgb, las_feas))
                xyz = xyz.astype(np.float32)
                sub_feas = sub_feas.astype(np.float32)
                labels = labels.astype(np.int32)

                # Subsample cloud
                sub_points, sub_feas, sub_labels = grid_subsampling(xyz,
                                                                      features=sub_feas,
                                                                      labels=labels,
                                                                      sampleDl=dl)
                ...
                write_ply(sub_ply_file,
                          [sub_points, sub_feas, sub_labels],
                          ['x', 'y', 'z', 'red', 'green', 'blue', 'ReturnNumber', 'NumberOfReturns', 'Reflectance', 'class'])

            # Fill data containers
            self.input_trees += [search_tree]
            self.input_colors += [sub_feas]
            self.input_labels += [sub_labels]

            size = sub_feas.shape[0] * 4 * 7
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    ...
                    idxs = self.input_trees[i].query(xyz, return_distance=False)
    ...
    def load_evaluation_points(self, file_path):
        ...
        xyz, rgb, rawlabels = read_las(file_path, False, True)
        xyz = xyz.astype(np.float32)
        return xyz
...
class Hessigsim3DSampler(Sampler):
    """Sampler for Hessigsim3D"""

    def __init__(self, dataset: Hessigsim3DDataset):
...
class Hessigsim3DCustomBatch:
    """Custom batch definition with memory pinning for Hessigsim3D"""
...
def Hessigsim3DCollate(batch_data):
    return Hessigsim3DCustomBatch(batch_data)
