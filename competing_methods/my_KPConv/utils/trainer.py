...
class ModelTrainer:
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        # if val_loader.dataset.filvalidation_split not in val_loader.dataset.all_splits:
        #     return
        if len(val_loader.dataset.files) == 0:
            return

        # Number of classes including ignored labels
        nc_tot = val_loader.dataset.num_classes

        # Number of classes predicted by the model,  not include ignored labels
        ...
        if config.saving:
            ...
            for i, file_path in enumerate(files):
                ...
                cloud_name = os.path.splitext(os.path.basename(file_path))[0] #file_path.split('/')[-1]
