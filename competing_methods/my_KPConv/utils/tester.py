...
def write_our_ply(filename, xyz, label):
    """write into a ply file"""
    prop = [('x', 'f8'), ('y', 'f8'), ('z', 'f8'), ('red', 'u1'), ('green', 'u1'), ('blue', 'u1'), ('Classification', 'u1')]
    colors = COLOR_MAP[np.asarray(label)]
    vertex_all = np.empty(len(xyz), dtype=prop)
    for i_prop in range(0, 3):
        vertex_all[prop[i_prop][0]] = xyz[:, i_prop]
    for i_prop in range(0, 3):
        vertex_all[prop[i_prop + 3][0]] = colors[:, i_prop]
...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        # Number of classes predicted by the model
        nc_model = config.num_classes - len(test_loader.dataset.ignored_labels)
        ...
        while True:
            ...
            if last_min + 1 < new_min or test_epoch >= config.max_test_epoch:
                ...
                if int(np.ceil(new_min)) % 10 == 0 or test_epoch >= config.max_test_epoch:
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        # Save plys
                        cloud_name = os.path.splitext(os.path.basename(file_path))[0] # + ".ply" # file_path.split('/')[-1]
                        ...
                        # points[:, 1] += 5426490.00
                        # points[:, 2] += 224.51
                        # write_our_ply(test_name, points, preds)

                        # for our SUM benchmark
                        write_ply(test_name,
                                  [points, preds],
                                  ['x', 'y', 'z', 'preds'])
                        # ---------------------------------
            ...
            print("last_min: " + str(last_min) + "; num_votes: " + str(num_votes))
