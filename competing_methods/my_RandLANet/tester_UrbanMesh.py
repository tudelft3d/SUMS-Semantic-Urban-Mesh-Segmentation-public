...
################################### UTILS Functions #######################################
COLOR_MAP = {
    0: (170, 85, 0), # 'ground' -> brown
    1: (0, 255, 0),  # 'vegetation' -> green
    2: (255, 255, 0),# 'building' -> yellow
    3: (0, 255, 255),# 'water' -> blue
    4: (255, 0, 255),# 'vehicle'/'car' -> pink
    5: (0, 0, 153)   # 'boat' -> purple
}
...
def write_ply_with_plyfilelib(filename, xyz, rgb, labels):
            ...
            try:
                ...
                if last_min + 4 < new_min:
                    ...
                    for i, file_path in enumerate(files):
                        ...
                        # Save plys
                        xyz, rgb = read_ply_with_plyfilelib(file_path)
                        labels = np.squeeze(preds)

                        for li, ind in zip(labels, range(len(labels))):
                            for c in COLOR_MAP:
                                if li == c:
                                    rgb[ind] = COLOR_MAP[c]
                                    break
                        ...
                        # Save to ply
                        file_name = os.path.splitext(os.path.basename(file_path))[0] + "_pred.ply"
                        ply_file = join(test_path, 'predictions', file_name)
                        write_ply_with_plyfilelib(ply_file, xyz, rgb, labels)
                        ...
                        log_string(cloud_name + ' has saved', self.log_out)
                        i_test += 1
