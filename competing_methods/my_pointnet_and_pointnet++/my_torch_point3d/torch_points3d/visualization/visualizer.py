...
class Visualizer(object):
    ...
    def __init__(self, viz_conf, num_batches, batch_size, save_dir):
        ...
        #add only visualized data
        try:
            self._vis_data = getattr(viz_conf, "vis_data", None)
        except:
            self._vis_data = None
        self._vis_file_name = ""
    ...
    def get_file_names(self, raw_folder, stage_name, data_i):
        if stage_name == "val":
            new_stage_name = "validate"
        else:
            new_stage_name = stage_name
        data_folder = raw_folder + "/" + new_stage_name + "/"
        files = glob.glob(data_folder + "*.ply")
        self._vis_file_name = os.path.splitext(os.path.basename(files[data_i]))[0]

    def get_indices(self, stage):
        """This function is responsible to calculate the indices to be saved"""
        if self._contains_indices or stage not in self._vis_data:
            return
        ...
        if self._stage in self._indices:
            ...
            for idx in np.argwhere(self._seen_batch == batch_indices).flatten():
                ...
                for visual_name, item in visuals.items():
                    ...
                    filename = "{}.ply".format(self._vis_file_name)
